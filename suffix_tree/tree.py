...
class Tree(lca_mixin.Tree):
    ...
    def findPosition(self, iterable):
        ...
        if dummy_child is not None:
            return matched_len == len(path), dummy_child.get_positions()
        else:
            return False, None
    ...
    def to_SuffixTrie(self):
        """ Output the tree in GraphViz .dot format. """
        dot = []
        dot.append('strict digraph G {\n')
        self.root.to_SuffixTrie(dot)
        dot.append('}\n')
        return ''.join(dot)
